...
<body>
    ...
    <?php
  // Retrieve the data
  $sql = "SELECT * FROM alumni_feedback_system.view1";
  ...
  $sql = "SELECT DISTINCT rollno FROM alumni_feedback_system.view1 ORDER BY rollno ASC";
  $result = mysqli_query($conn, $sql);
  ...
  $sql = "SELECT DISTINCT alumniname FROM view1 ORDER BY alumniname ASC";
  $result = mysqli_query($conn, $sql);
  $alumniname_options = "<option value=''>All Alumni's</option>"; // Add an option to select all rows
  while ($row = mysqli_fetch_assoc($result)) {
    $selected = ($row["alumniname"] == $_POST["alumniname_filter"]) ? "selected" : ""; // Check if this option is selected
    $alumniname_options .= "<option value='" . $row["alumniname"] . "' $selected>" . $row["alumniname"] . "</option>";
  }
  ...
  echo "<label>Alumni Name:</label>";
  echo "<select name='alumniname_filter'>";
  echo $alumniname_options;
  echo "</select>";
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $alumniname_filter = $_POST["alumniname_filter"];
    ...
    $sql = "SELECT * FROM view1 WHERE rollno LIKE '%$rollno_filter%' AND alumniname LIKE '%$alumniname_filter%'
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      echo "<tr><td>" . $row["rollno"] . "</td><td>" . $row["alumniname"] . "</td><td>" .
  ...
  ?>
    ...
    function clearFilters() {
        ...
        document.getElementsByName("alumniname_filter")[0].value = "";
