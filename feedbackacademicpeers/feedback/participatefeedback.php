    ...
    <section class="content">
        <form method="post" action="">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <?php
$sql = "SELECT * FROM `feedbacktopic` WHERE course_id in ((select course_id from peers where peersid = $_SESSION[peersid]), (select course_id from course where course_title='All'))";
$qsql = mysqli_query($con, $sql);

echo mysqli_error($con);
$img = "";
while ($rs = mysqli_fetch_array($qsql)) {
    // $sqlpeers = "SELECT * FROM peers  WHERE peers_id='$rs[peers_id]'";
    ...
    // if ($rs['peers_id'] != 0) {
    //     echo "<hr><img src='imgpeers/" . $rspeers['peers_img'] . "' style='width: 50px; height: 50px;' align='left' > &nbsp;<b>" . $rspeers['peers_name'] . "</b><br> &nbsp;" . $rspeers['peers_designation'] . "";
    // }
    echo "</td>";
    echo "<td>" . date("d-M-Y", strtotime($rs['feedbacktopic_date'])) . "<br>" . date("h:i A", strtotime($rs['feedbacktopic_date'])) . "</td>";
    echo "<td>";
    $sqlfeedbacktest = "select * from feedbackquestion_result WHERE feedbacktopicid='$rs[feedbacktopicid]' AND peersid='$_SESSION[peersid]'";
    $qsqlfeedbacktest = mysqli_query($con, $sqlfeedbacktest);
    if (mysqli_num_rows($qsqlfeedbacktest) >= 1) {
        echo "<a href='feedbackquestion_success.php?feedbacktopicid=$rs[0]&peersid=$_SESSION[peersid]' class='btn btn-success'  >View Entries</a>";
    } else {
