            ...
            <div class="card">
                ...
                <div class="card-body">
                    <table id="myTable" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            <?php
$sql = "SELECT * FROM feedbackquestion_result GROUP BY employeeid";
...
while ($rs = mysqli_fetch_array($qsql)) {
    ...
    //######
    /*
    $sqlqz ="SELECT * FROM feedbackquestion_result WHERE feedbacktopicid='$rs[feedbacktopicid]' AND employeeid='$rs[employeeid]' and selectedanswer = correctanswer ";
    $qsqlqz  = mysqli_query($con,$sqlqz);
    echo mysqli_error($con);
    $correctanswer= mysqli_num_rows($qsqlqz);
     */

    $sqlfeedbackquestion_result = "SELECT * FROM feedbackquestion_result WHERE feedbacktopicid='$rs[feedbacktopicid]' AND employeeid='$rs[employeeid]'";
    ...
    echo "<tr>
				<td>";
    ?>
                            <b><?php echo $rsemployee['employeename']; ?></b><br>
                            <!-- <?php echo $rsemployee['rollno']; ?> -->
                            <?php
echo "</td>
				<td>";
    ?>
                            <?php
$sqlcourse = "SELECT * FROM course where course_id='$rsemployee[course_id]'";
    $qsqlcourse = mysqli_query($con, $sqlcourse);
    $rscourse = mysqli_fetch_array($qsqlcourse);
    echo $rscourse['course_title'];?>
                            <!-- (<?php echo $rsemployee['section']; ?>) -->
                            <?php
echo "</td>
				<td>" . date("d-M-Y", strtotime($rs['date'])) . "</td>";
    if (isset($_SESSION['adminid'])) {
        echo "<td><a href='admin_feedbackquestion_success.php?feedbacktopicid=$rs[feedbacktopicid]&employeeid=$rs[employeeid]' class='btn btn-info'>View</a> </td>";
    } else {
        echo "<td><a href='feedbackquestion_success.php?feedbacktopicid=$rs[feedbacktopicid]&employeeid=$rs[employeeid]' class='btn btn-info'>View</a> </td>";
    }
    echo "</tr>";
}
?>
